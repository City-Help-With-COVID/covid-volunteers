module ApplicationHelper
  ...
  def nav_link_active_class(variant = 'DESKTOP')
    case variant
    when 'DESKTOP'
      'inline-flex items-center px-1 pt-1 border-b-2 border-<%= PRIMARY_COLOR %>-500 text-sm leading-5 text-gray-900 focus:outline-none focus:border-<%= PRIMARY_COLOR %>-700 transition duration-150 ease-in-out ml-4 text-center'
    when 'MOBILE'
      'block pl-3 pr-4 py-2 border-l-4 border-<%= PRIMARY_COLOR %>-500 text-base text-<%= PRIMARY_COLOR %>-700 bg-<%= PRIMARY_COLOR %>-50 focus:outline-none focus:text-<%= PRIMARY_COLOR %>-800 focus:bg-<%= PRIMARY_COLOR %>-100 focus:border-<%= PRIMARY_COLOR %>-700 transition duration-150 ease-in-out'
    end
  end
